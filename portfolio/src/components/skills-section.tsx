            ...
            <div className="container mx-auto max-w-6xl relative z-10">
                ...
                <div className="space-y-12">
                    {skillCategories.map((category, idx) => (
                        <div key={idx} className={`text-center ${isVisible ? "animate-in fade-in slide-in-from-bottom duration-700 fill-mode-backwards" : "opacity-0"}`} style={{ animationDelay: isVisible ? `${idx * 200}ms` : '0ms' }}>
                            ...
                            <div className="flex flex-wrap justify-center gap-4">
                                {category.skills.map((skill, sIdx) => {
                                    ...
                                    return (
                                        <div
                                            key={sIdx}
                                            className={`w-14 h-14 md:w-16 md:h-16 rounded-xl overflow-hidden flex items-center justify-center transform hover:scale-110 transition-transform cursor-default group relative ${isVisible ? "animate-in zoom-in fade-in duration-500 fill-mode-backwards" : "opacity-0"}`}
